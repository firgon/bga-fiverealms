...
define([
  ...
  return declare("bgagame.fiverealms", [customgame.game], {
    ...
    setupDiscardModal() {
      ...
      $("discard-cards-holder").addEventListener("click", () =>
    ...
    onEnteringStatePlay(args) {
      if (args.deck) {
        this.addCard(args.deck);
        this.slide(`card-${args.deck.id}`, "fiverealms-top-deck");
      }
      if (args.secondCardOnDeck) {
        this.addCard(args.secondCardOnDeck);
      }
    ...
    onEnteringStatePlayChooseAction(args) {
      ...
      if (args.realm == "imperial") {
        ...
        REALMS.forEach((realm) => {
          if (this._counters[this.player_id][realm].getValue() != 0) {
            canInfluence = true;
          }
        });
      }
    ...
    notif_passChooseCharacter(n) {
      ...
      ).then(() => $("resizable-central-board").classList.remove("recruiting"));
    },
