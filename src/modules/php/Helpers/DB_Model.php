...
abstract class DB_Model extends \APP_DbObject implements \JsonSerializable
{
  ...
  /**
   * This array will contains class attributes that will be bind depending on flipped attribute and $is_current
  ...
  /**
   * bind attribute if $isCurrent == false AND isflipped (or we don't know)
   */
  public function getUiData($isCurrent = false)
  {
    ...
    foreach ($data as $attribute => $value) {
      if (in_array($attribute, array_keys($this->bindedAttributes))) {
        if (!$isCurrent && (!isset($this->flipped) || $this->isFlipped())) {
          $data[$attribute] = $this->bindedAttributes[$attribute];
        }
      }
    }
