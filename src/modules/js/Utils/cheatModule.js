...
define(['dojo', 'dojo/_base/declare', 'dojo/fx', 'dojox/fx/ext-dojo/complex'], function (dojo, declare) {
	...
	return declare('fiverealms.cheatModule', null, {

		cheatModuleSetup(gamedatas){
			dojo.place(this.cheatBlock(), "player_boards");
          Object.entries(gamedatas.cheatModule).forEach(([name, data]) => this.addPanel(name,data));
	  ...
	  addPanel(name, datas){
		...
		dojo.connect($('send_'+name), 'click', (e) => {
			...
			debug("je vais envoyer :", {data: Object.fromEntries(data.entries())});
			...
			this.takeAction('cheat', {data: JSON.stringify(Object.fromEntries(data.entries()))}, false, false );
		}
		);
	  },
	  ...
	  createInput(label, data, subs=null){
		...
		let subFields = [];
		if (Array.isArray(data)){//simple array data
			Object.values(data).forEach((name)=> result+=`<option value='${name}'>${name}</option>`);
		}else {
			Object.entries(data).forEach(([id, data])=> {
				...
				} else {// simple id -> name format data
